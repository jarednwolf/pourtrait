...
export default function DashboardPage() {
  ...
  const [summary, setSummary] = React.useState<{ totalWines: number; hasTasteProfile: boolean } | null>(null)
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) setSummary({ totalWines: json?.totalWines || 0, hasTasteProfile: !!json?.hasTasteProfile })
      } catch {}
    }
    run()
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:py-6">
        ...
        {summary && (
          <p className="mt-1 text-sm text-gray-600" aria-live="polite">
            {summary.totalWines} {summary.totalWines === 1 ? 'bottle' : 'bottles'} • {summary.hasTasteProfile ? 'Profile calibrated' : 'Profile not calibrated'}
          </p>
        )}
        <div className="mt-4">
          <PrimaryActionRow />
